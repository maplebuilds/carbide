...
function Flag({ type, children, theme }: { type: 'green' | 'yellow' | 'red'; children: React.ReactNode; theme: 'dark' | 'light' }) {
  ...
  return (
    <div className="flex gap-2.5 items-start px-3 py-2.5 rounded-lg text-[15px] leading-relaxed mb-2 last:mb-0"
      style={{ background: s.bg, border: `1px solid ${s.border}` }}>
      <span className="flex-shrink-0 text-smmt-0.5">{s.icon}</span>
      <span className={theme === 'dark' ? 'text-white/70' : 'text-black/70'} style={{ fontFamily: "'Martel', Georgia, serif" }}>{children}</span>
    </div>
  );
}
...
function CostRow({ label, value, highlight, theme }: { label: string; value: string; highlight?: boolean; theme: 'dark' | 'light' }) {
  ...
  return (
    <div className={`flex justify-between items-baseline py-2.5 border-b last:border-0 ${isDark ? 'border-white/[0.06]' : 'border-black/[0.06]'}`}>
      <span className={`text-base${highlight ? (isDark ? 'text-white font-medium' : 'text-[#0e0e10] font-medium') : (isDark ? 'text-white/50' : 'text-black/50')}`} style={{ fontFamily: "'Martel', Georgia, serif" }}>{label}</span>
      <span className={`font-data text-basefont-medium ${highlight ? 'text-[#00B4FF]' : isDark ? 'text-white' : 'text-[#0e0e10]'}`}>{value}</span>
    </div>
  );
}
...
function Section({ label, children, theme }: { label: string; children: React.ReactNode; theme: 'dark' | 'light' }) {
  ...
  return (
    <div className={`rounded-xl p-6 ${isDark ? 'steel-card' : 'light-card'}`}>
      <p className={`font-data text-[12px] uppercase tracking-[0.12em] mb-4 ${isDark ? 'text-white/30' : 'text-black/30'}`}>{label}</p>
      {children}
    </div>
  );
}
...
function Stat({ label, value, accent, dim, theme }: { label: string; value: string; accent?: boolean; dim?: boolean; theme: 'dark' | 'light' }) {
  ...
  return (
    <div>
      <p className={`font-data text-[12px] uppercase tracking-[0.08em] mb-1 ${isDark ? 'text-white/30' : 'text-black/30'}`}>{label}</p>
      <p className={`font-data text-xlfont-medium ${accent ? 'text-[#00B4FF]' : dim ? (isDark ? 'text-white/50' : 'text-black/50') : isDark ? 'text-white' : 'text-[#0e0e10]'}`}>
        {value}
      </p>
    </div>
  );
...
function AnalyzeButton({ onClick, loading, error, theme }: { onClick: () => void; loading: boolean; error?: boolean; theme: 'dark' | 'light' }) {
  ...
  return (
    <>
      {error && (
        <p className="mt-3 text-center font-data text-[13px] text-red-400/80">Analysis failed — tap to retry</p>
      )}
      <button
        onClick={onClick}
        disabled={loading}
        className={`mt-2 w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border font-data text-[13px] uppercase tracking-wider transition-all active:scale-95 ${
          loading
...
function AffiliateRow({ theme, links }: {
  ...
  const isDark = theme === 'dark';
  return (
    <div className={`grid gap-2.5 mt-4 ${links.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
      {links.map(link => {
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between gap-1">
              <span className="font-data text-[13px] font-semibold uppercase tracking-wider"
                style={{ color: hex }}>{link.label}</span>
              <ExternalLink size={11} style={{ color: hex, flexShrink: 0 }} />
            </div>
            <span className={`font-data text-[13px] leading-snug ${isDark ? 'text-white/50' : 'text-black/50'}`}>
              {link.sublabel}
            </span>
          </a>
        );
...
export default function CarReport({ report, vehicle, theme }: CarReportProps) {
  ...
  const verdictMileageRef = useRef<number | null>(null);
  ...
  // Generate verdict whenever mileage is set or changes
  useEffect(() => {
    if (actualMileage && actualMileage !== verdictMileageRef.current) {
      verdictMileageRef.current = actualMileage;
      setSectionData(prev => ({ ...prev, verdict: undefined }));
      setSectionErrors(prev => ({ ...prev, verdict: false }));
      analyzeSection('verdict');
    }
  }, [actualMileage, analyzeSection]);
  ...
  return (
    <div className="relative">
      ...
      <div className="relative z-10 space-y-4">
        ...
        {(() => {
          ...
          return (
            <div className={`rounded-xl border-2 border-[#00B4FF] px-6 py-6 overflow-hidden ${isDark ? 'bg-[#00B4FF]/[0.04]' : 'bg-[#00B4FF]/[0.04]'}`}>
              <p className="font-data text-[12px] text-[#00B4FF] uppercase tracking-[0.15em] mb-1">
                {vehicle.year} · {vehicle.make}
              </p>
              <div className="min-w-0">
                <h1 className="font-display text-[28px] leading-tight" style={{ color: isDark ? '#e8e8ec' : '#0e0e10', overflowWrap: 'anywhere' }}>
                  {vehicle.model}
                  {vehicle.trim && <em className="text-[#00B4FF] not-italic"> {vehicle.trim}</em>}
                </h1>
              </div>
              <div className={`flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 font-data text-[13px] ${isDark ? 'text-white/25' : 'text-black/25'}`}>
              ...
              <div className="mt-3 space-y-3">
                {(() => {
                  const pillLabels = ['Walk Away', 'Proceed with Caution', 'Fair Buy', 'Smart Buy'];
                  const pillLabel = pillLabels[score];
                  const pillBg = activeColor + '1a';
                  const pillBorder = activeColor + '40';
                  const Icon = score >= 2 ? CheckCircle : XCircle;
                  return (
                    <span className="inline-flex items-center gap-1.5 font-data text-[13px] px-2.5 py-1 rounded-full"
                      style={{ color: activeColor, background: pillBg, border: `1px solid ${pillBorder}` }}>
                      <Icon size={11} /> {pillLabel}
                    </span>
                  );
                })()}
                ...
                <div className="pt-2 px-2">
                  ...
                  <div className="relative w-full h-[3px] rounded-full mb-5"
                    ...
                    <div className="absolute w-6 h-6 rounded-full border-2 -top-[11px] transition-all duration-300"
                      style={{ left: `calc(${(score / 3) * 100}% - 12px)`,
                  ...
                  <div className="flex justify-between">
                    {stops.map((stop, i) => (
                      <span key={i} className="font-data text-[11px] leading-none whitespace-nowrap"
                        style={{ color: i === score ? activeColor : isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)',
        ...
        <div className={`rounded-xl p-6 ${isDark ? 'steel-card' : 'light-card'}`}>
          <p className={`font-data text-[12px] uppercase tracking-[0.12em] mb-3 ${isDark ? 'text-white/30' : 'text-black/30'}`}>
            Your Numbers — Optional
          </p>
          <div className="flex gap-2">
            <div className="relative flex-1">
              <span className={`absolute left-3 top-1/2 -translate-y-1/2 font-data text-basepointer-events-none ${isDark ? 'text-white/30' : 'text-black/30'}`}>$</span>
              <input
                ...
                className={`w-full pl-7 pr-4 py-2.5 rounded-lg font-data text-baseborder outline-none focus:border-[#00B4FF] transition-colors ${
          ...
          <div className="flex gap-2 mt-2">
            <div className="relative flex-1">
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-lg font-data text-baseborder outline-none focus:border-[#00B4FF] transition-colors ${
                  mileageSaved
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-lg font-data text-smuppercase tracking-wider active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap flex items-center gap-1.5"
              style={{ background: mileageSaved ? 'rgba(0,180,255,0.12)' : '#00B4FF',
          ...
          {dealAnalysis && analyzedDealerPrice !== null && (
            <div className="mt-3 flex items-center justify-between gap-3 pt-3 border-t"
              ...
              <p className="font-display text-[24px]" style={{ color: dealAnalysis.color }}>{dealAnalysis.rating}</p>
              <div className="text-right">
                <p className="font-data text-basefont-medium" style={{ color: dealAnalysis.color }}>{dealAnalysis.deltaLabel}</p>
                <p className={`font-data text-[13px] mt-0.5 ${isDark ? 'text-white/25' : 'text-black/25'}`}>
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <div className={`rounded-xl p-5 ${isDark ? 'steel-inner' : 'light-inner'}`}>
            <p className={`font-data text-[12px] uppercase tracking-[0.1em] mb-2 ${isDark ? 'text-white/30' : 'text-black/30'}`}>
              {analyzedDealerPrice ? 'Monthly (your price)' : 'Monthly Payment'}
            </p>
            <p className="font-data text-[24px] font-medium leading-none text-[#00B4FF]">
              {monthly60}<span className={`text-smml-1 ${isDark ? 'text-white/30' : 'text-black/30'}`}>/mo</span>
            </p>
            <p className={`text-[13px] mt-1.5 ${isDark ? 'text-white/25' : 'text-black/25'}`}>good credit · 60 months</p>
          </div>
          <div className={`rounded-xl p-5 ${isDark ? 'steel-inner' : 'light-inner'}`}>
            <p className={`font-data text-[12px] uppercase tracking-[0.1em] mb-2 ${isDark ? 'text-white/30' : 'text-black/30'}`}>3-Year TCO</p>
            <p className="font-data text-[24px] font-medium leading-none text-[#00B4FF]">
              {formatDollar(year3Total)}
            </p>
            <p className={`text-[13px] mt-1.5 ${isDark ? 'text-white/25' : 'text-black/25'}`}>total cost of ownership</p>
          </div>
        </div>

        {/* ── Cost breakdown ── */}
        <div className={`rounded-xl p-6 ${isDark ? 'steel-card' : 'light-card'}`}>
          <p className={`font-data text-[12px] uppercase tracking-[0.12em] mb-3 ${isDark ? 'text-white/30' : 'text-black/30'}`}>Cost Breakdown</p>
          ...
          <AffiliateRow theme={theme} links={[
            { label: 'LendingTree', sublabel: `Est. ${monthly60}/mo · good credit, 60mo`, href: 'https://www.lendingtree.com/auto/?source=carbide', trackingId: 'financing-cta', accent: '#22c55e' },
            { label: 'The Zebra', sublabel: `Est. ${report.insuranceEstimate.monthlyLow}–${report.insuranceEstimate.monthlyHigh}/mo insurance`, href: 'https://www.thezebra.com/?source=carbide', trackingId: 'insurance-cta', accent: '#a78bfa' },
          ]} />
        </div>
        ...
        <div className={`rounded-xl border-2 p-6 ${report.bottomLine.smartBuy ? 'border-[#00B4FF]/40 bg-[#00B4FF]/[0.03]' : 'border-amber-500/40 bg-amber-500/[0.03]'}`}>
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <p className={`font-data text-[12px] uppercase tracking-[0.12em] ${isDark ? 'text-white/30' : 'text-black/30'}`}>The Verdict</p>
          </div>
          {sectionData.verdict ? (
            <>
              {sectionData.verdict.overview && (
                <p className={`text-baseleading-relaxed mb-3 ${isDark ? 'text-white/55' : 'text-black/55'}`} style={{ fontFamily: "'Martel', Georgia, serif" }}>{sectionData.verdict.overview}</p>
              )}
        ...
        {/* ── Flags ── */}
        <div className={`rounded-xl p-6 ${isDark ? 'steel-card' : 'light-card'}`}>
          <p className={`font-data text-[12px] uppercase tracking-[0.12em] mb-3 ${isDark ? 'text-white/30' : 'text-black/30'}`}>Flags</p>
